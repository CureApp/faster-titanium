...
var MainProcess = function () {
    ...
    function MainProcess(projDir) {
        ...
        process.on('exit', function (x) {
            ...
            console.log(_chalk2.default.yellow('faster-ti restart -f ' + fPort + ' -n ' + nPort + ' -p ' + platform + ' ' + projDir));
        });
    }
    ...
    _createClass(MainProcess, [{
        ...
        value: function onAlloyFileChanged(path) {
            ...
            /** @type {AlloyCompiler} */
            return this.alloyCompiler.compile(path, token).then(function (x) {
                return _this.watcher.removeListener('change:Resources', poolChanged);
            }).then(function (x) {
                return _this.send({ event: 'alloy-compilation-done', token: token });
            }).then(function (x) {
