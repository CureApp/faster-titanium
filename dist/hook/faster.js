...
function init(logger, config, cli) {
    ...
    var hooks = [['build.config', attachFasterFlag.bind(scope)], ['build.pre.compile', filter.call(scope, launchServers)], // attaches scope.ftProcess
...
function filter(fn) {
    ...
    return function (data, finished) {
        ...
        if (!isAlloyCompatible.call(_this)) {
            process.exit(1);
        }
