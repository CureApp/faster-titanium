...
function modifyEntryName(data) {
    ...
    var isAppJS = ['', 'android', 'ipad', 'iphone', 'ios'].map(function (name) {
        ...
        return _this.logger.info(path) || src === path;
    });
...
function createAppJS(dist) {
    ...
    var codeToRun = 'Ti.FasterTitanium.run(' + JSON.stringify(optsForFasterTi) + ')';

    var appJSPath = (0, _path.resolve)(__dirname, '../../dist/app.js');
    var appJSCode = [_fs2.default.readFileSync(appJSPath, 'utf8'), codeToRun].join('\n');

    this.logger.info('[FasterTitanium] Creating new app.js with host: ' + this.host + ', fPort: ' + fPort + ', ePort: ' + ePort);

    _fs2.default.writeFileSync(dist, appJSCode);
}
...
function launchServer(data) {
    ...
    var _cli$argv4 = this.cli.argv;
    var fPort = _cli$argv4['faster-port1'];
    var ePort = _cli$argv4['faster-port2'];
