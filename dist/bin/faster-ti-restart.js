...
_commander2.default.arguments('[proj-dir]').option('-f, --fport <port number>', 'port number of the http server', parseInt).option('-n, --nport <port number>', 'port number of the notification server', parseInt).option('-p, --platform <platform name>', 'ios|android').parse(process.argv);
...
function run() {
    ...
    var hasOptions = ['fport', 'nport', 'platform'].every(function (opt) {
...
function showLogo() {
    log((0, _fs.readFileSync)(__dirname + '/../../doc/ft-txt-logo', 'utf8'), 'green');
